//! The spinner on a real terminal: a thread, a clock, and the bytes [`clackatui_core::spinner`]
//! produces.
//!
//! ```no_run
//! let spinner = clackatui::spinner().start("Installing dependencies");
//! // ...
...
use clackatui_core::spinner::{self as core_spinner, Indicator, StyleFrame};
use clackatui_core::theme::{Theme, unicode_supported};
...
impl Builder {
	...
	/// [`Line`](clackatui_core::frame::Line), so it can add characters as well as colour.
