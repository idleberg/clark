...
[`clark_core::task_log`](../../crates/clark-core/src/task_log.rs) is the port.
...
Saying that without escapes is what [`crate::message::log_lines`](../../crates/clark-core/src/message.rs)
...
- **The driver has no thread.** `progress` reused the spinner's interval; a task log needs no
  interval, so `crates/clark/src/task_log.rs` is a builder, a struct and eleven methods that print.
