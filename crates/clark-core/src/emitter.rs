...
//! path. It still governs the `Widget` impl, which is what makes a clark Prompt drawable inside
...
///
/// Produces the bytes rather than writing them, so that `clark-core` stays free of I/O. The
/// driver in `clark` is what puts them on a terminal.
#[derive(Clone, Debug)]
pub struct Emitter {
