...
//!
//! ```no_run
//! let name = clark::text("What is your name?").interact()?;
//! # Ok::<_, clark::ClackError>(())
//! ```
//!
//! The state machines, the Frames and the Emitter are all in `clark-core`, which performs no
...
//! `clark-core` against a harvested recording of the real library. What this crate adds is the
...
pub use clark_core::progress::BarStyle;
/// The pieces a caller needs to configure a Prompt or to drive one themselves.
pub use clark_core::prompt::{Outcome, Validator};
pub use clark_core::select::SelectOption;
pub use clark_core::session::Session;
pub use clark_core::settings::{Action, DateMessages, Settings};
pub use clark_core::spinner::{Indicator, StyleFrame};
pub use clark_core::theme::Theme;
