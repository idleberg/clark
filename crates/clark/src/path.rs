...
//! [`clark_core::path::options`].
//!
//! # This is the crate that touches a disk
//!
//! `clark-core` performs no I/O, so the reading is here: [`StdFs`] answers
//! [`Fs`](clark_core::path::Fs) with `std::fs`, and is the only thing in the port that opens a
...
use clark_core::autocomplete::{AutocompleteState, AutocompleteWidget};
use clark_core::path::{Fs, options, required};
...
///
/// ```no_run
/// let file = clark::path("Select a file").root("/tmp").interact()?;
