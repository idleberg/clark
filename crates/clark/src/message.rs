...
//!
//! ```no_run
//! clark::intro("create-app");
//! clark::log::success("dependencies installed");
//! clark::outro("You're all set!");
//! ```
//!
...
use clark_core::r#box as core_box;
use clark_core::emitter::write_once;
use clark_core::frame::Span;
use clark_core::message;
use clark_core::note as core_note;
...
///
/// The formatter returns a drawn [`Line`](clark_core::frame::Line) where upstream's returns a
/// string, so it can add colour as well as characters — see [`clark_core::note`].
pub fn note_with(
...
/// than the width of the content. See [`clark_core::box`] for why, and use [`box_with`] to ask
/// for what the documentation promises.
///
/// ```no_run
/// clark::r#box("It fills the terminal.", "Heads up");
/// ```
...
///
/// ```no_run
/// use clark_core::r#box::{Align, Options, Width};
/// # let theme = clark::Theme::clack();
/// let options = Options {
///     width: Width::Auto,
...
///     ..Options::default()
/// };
/// clark::box_with("Centred, and only as wide as it needs to be.", "Heads up", &theme, &options);
/// ```
pub fn box_with(
	message: impl AsRef<str>,
	title: impl AsRef<str>,
